...
本项目的首页作品列表和文章详情页都由本地 MDX 文件驱动。你只需要管理 `content/articles/` 目录里的 `.mdx` 文件，不需要改 React 代码。
...
可以写段落、列表、图片、链接，也可以后续接入自定义 React 组件。
```
...
- `cover`：首页卡片封面图，可以是远程 URL，也可以是 `/public` 下的本地图片路径。
- `coverAlt`：封面图描述，建议填写。
- `order`：可选，用于手动调整首页排序。
